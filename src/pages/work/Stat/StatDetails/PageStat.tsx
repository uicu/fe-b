...
import { getWorkStatAverageTimeService, getWorkStatListService } from '../../../../services/stat'
...
const PageStat: FC<PropsType> = (props: PropsType) => {
  ...
  // 加载列表
  const { loading, run: runGetWorkStatListService } = useRequest(
    async () => {
      const res = await getWorkStatListService(id, { pageNo, pageSize })
      return res
    },
    {
  ...
  // 加载平均完成时间
  const { loading: loadingAverageTime, run: runGetWorkStatAverageTimeService } = useRequest(
    async () => {
      const res = await getWorkStatAverageTimeService(id)
      return res
    },
    {
      refreshDeps: [id],
  ...
  const handelRefresh = (initPageNo = true) => {
    if (pageNo !== 1 && initPageNo) {
      ...
      runGetWorkStatListService()
    }
    runGetWorkStatAverageTimeService()
  }
